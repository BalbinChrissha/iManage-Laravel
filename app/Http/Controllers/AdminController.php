...
class AdminController extends Controller
{
    ...
    public function admin_Dashboard()
    {
        ...
        return view('admin.adminDashboard', [ 'available' => $available, 'decommissioned' => $decommissioned, 'repair'=>$repair, 'availableCat2' =>  $available2, 'consumed' =>  $consumed, 'expired' => $expired, 'remainder' => $remainder , 'totalqty1'=> $totalqty1 ,'totaltransfer' => $totaltransfer, 'remainder2' =>  $remainder2, 'totalqty2' => $totalqty2,'totaltransfer2'=> $totaltransfer2]);
    ...
    public function manageAdmin()
    {
        ...
        return view('admin.manageAdmin', ['admins' => $admins]);
    }


    public function deleteAdmin($id)
    {
        $admins = DB::delete('delete from admin where adminID=?', [$id]);
    ...
    public function addAdministrator(Request $request)
    {
        ...
        $insert = DB::insert('insert into admin (admin_name, username, password) values (?, ?, ?)', [$name, $username, $password]);
    ...
    {
        $admin = DB::select('select * from admin where adminID=?', [$id]);
        $admin = $admin[0];
        $adminId = $admin->adminID;
        $name = $admin->admin_name;
        $username = $admin->username;
        $password = $admin->password;
        return view('admin.adminUpdate', ['adminId' =>  $adminId, 'name' => $name, 'username' => $username, 'password' => $password]);
    ...
    public function updateAdmin(Request $request, $id)
    {
        ...
        $update = DB::update('update admin set admin_name = ?, username = ?, password = ? where adminID = ?', [$name, $username, $password, $id]);
        if ($update) {
            return redirect()->route('admin.manageAdmin');
        }
    }
    ...
    public function manageSOS()
    {
        $staffs = DB::select('select * from supply_office_staff');
        return view('admin.manageSOS', ['staffs' => $staffs]);
    }

    public function addStaff(Request $request)
    {
        $employeenum = $request->input('employeenum');
        ...
        $insert = DB::insert('insert into supply_office_staff (staffID, staff_name, staff_username, staff_password, adminID) values (?, ?, ?, ?, ?)', [$employeenum, $name, $username, $password, $id]);

        if ($insert) {
            return redirect()->route('admin.manageSOS');
        }
    }
    ...
    {
        $admins = DB::delete('delete from supply_office_staff where staffID=?', [$id]);
        return redirect()->route('admin.manageSOS');
    }
    ...
    {
        $staff = DB::select('select * from supply_office_staff where staffID=?', [$id]);
        $staff = $staff[0];
        $staffId = $staff->staffID;
        $name = $staff->staff_name;
        $username = $staff->staff_username;
        $password = $staff->staff_password;
        return view('admin.staffUpdate', ['staffId' =>  $staffId, 'name' => $name, 'username' => $username, 'password' => $password]);
    }
    ...
    public function updateStaff(Request $request, $id)
    {
        ...
        $update = DB::update('update supply_office_staff set staff_name = ?, staff_username = ?, staff_password = ? where staffID = ?', [$name, $username, $password, $id]);
        if ($update) {
            return redirect()->route('admin.manageSOS');
        }
    }
    ...
    public function viewInventory()
    {
        ...
        $records = DB::select("select itemid, item.staffID,  category_name, item_name, serialno, item_desc, cost, date_purchased, quantity, item.categoryID, staff_name from item, supply_office_staff, category WHERE supply_office_staff.staffID =item.staffID AND item.categoryID = category.categoryID");
        return view('admin.viewInventory', ['records' => $records]);
    ...
    public function viewFaculty()
    {
        ...
        $records = DB::select("SELECT facultyID, dep_name, faculty_name, faculty_username, faculty_password, room_no, room_name FROM faculty_incharge, department WHERE faculty_incharge.departmentno = department.departmentno");
        return view('admin.viewFaculty', ['records' => $records]);
    ...
    public function filterInventory()
    {
        ...
        $records = DB::select('SELECT item.itemid, item_name, quantity, sum(qty_transferred) as totaltransfer, sum(available_qty) as available_qty,  month, year FROM item, category, item_transfer, item_last_checked , item_state WHERE category.categoryID=item.categoryID AND item.itemid=item_transfer.itemID AND item_transfer.recordno=item_state.recordno AND item_state.checkedID = item_last_checked.checkedID AND category.categoryID = 1 AND month=? AND year =? GROUP BY item.itemid, item.item_name, item.quantity, item_last_checked.month, item_last_checked.year' , [$currentMonth, $currentYear]);
        return view('admin.filterInventory', ['records' => $records, 'month' => $currentMonth, 'year' => $currentYear]);
